...
//map<SIGNAL, set<Object*>> Object::m_static_signal_observers = {};
...
/*void Object::emit_static_signal(SIGNAL signal, Object* subject){
	if(m_static_signal_observers.find(signal) == m_static_signal_observers.end()) return;

	for(auto obs : m_static_signal_observers[signal]){
		obs->update_on_signal(signal, subject);
	}
}

void Object::add_static_listener(SIGNAL signal, Object* observer){
	// add signal to list, if it doesnt exist
	if(m_static_signal_observers.find(signal) == m_static_signal_observers.end()){
		m_static_signal_observers.insert(pair<SIGNAL, OBSERVER_LIST>(signal, {}));
	}

	m_static_signal_observers[signal].insert(observer);
}
void Object::remove_static_listener(SIGNAL signal, Object* observer){
	// if signal is not in the list, it means object is definitely not there; unnecessary call, return
	if(m_static_signal_observers.find(signal) == m_static_signal_observers.end()) return;

	m_static_signal_observers[signal].erase(observer);

	// remove signal from list, if we just erased its last observer
	if(m_static_signal_observers[signal].empty()){
		m_static_signal_observers.erase(signal);
	}
}
*/
